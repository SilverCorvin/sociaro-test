...
def get_normalized_duration(seconds):
    """Принимает на вход время в секундах.
    ...
    def handle(self, *args, **options):
        ...
        for video in videos:
            ...
            try:
                ...
                video.duration = get_normalized_duration(
                    videodata._meta['duration'])
                video.preview = ImageFile(
                    tmpfile, os.path.basename(tmpfile.name))
                video.status = 1
                video.save()
            except:
                video.status = 4
